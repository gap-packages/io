                         ...
                         Obj timeoutusec)
{
    ...
    if (!IS_PLIST(inlist))
        ErrorMayQuit("<inlist> must be a list of small integers (not a %s)",
                     (Int)TNAM_OBJ(inlist), 0);
    if (!IS_PLIST(outlist))
        ErrorMayQuit("<outlist> must be a list of small integers (not a %s)",
                     (Int)TNAM_OBJ(outlist), 0);
    if (!IS_PLIST(exclist))
        ErrorMayQuit("<exclist> must be a list of small integers (not a %s)",
...
static Int InitLibrary(StructInitInfo * module)
{
    ...
    AssPRec(tmp, RNamName("IP_MULTICAST_IF"),
            INTOBJ_INT((Int)IP_MULTICAST_IF));
#endif
#ifdef SO_RCVBUF
    ...
    AssPRec(tmp, RNamName("SO_BINDTODEVICE"),
            INTOBJ_INT((Int)SO_BINDTODEVICE));
#endif
#ifdef SO_DEBUG
    ...
    AssPRec(tmp, RNamName("TCP_DEFER_ACCEPT"),
            INTOBJ_INT((Int)TCP_DEFER_ACCEPT));
#endif
#ifdef TCP_INFO
    ...
    AssPRec(tmp, RNamName("TCP_WINDOW_CLAMP"),
            INTOBJ_INT((Int)TCP_WINDOW_CLAMP));
#endif
#ifdef ICMP_FILTER
